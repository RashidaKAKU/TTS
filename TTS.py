...
class Application(tk.Tk):
    ...
    def init_ui(self):
        ...
        self.mp3_entry.pack(side=tk.TOP, fill=tk.X,
                            expand=True, padx=(5, 5), pady=0)

        self.row2_message = tk.Message(
            self.row2, text="上方输入MP3文件路径以 .mp3 结尾或点击右边 选择路径 按钮", width=350)
        self.row2_message.pack(side=tk.LEFT, anchor='w',expand=True, fill=tk.X, padx=0, pady=0)

        self.mp3_sel_button = tk.Button(
            self.row2, text="选择路径", command=self.select_mp3_path)
        self.mp3_sel_button.pack(side=tk.RIGHT, pady=5, padx=5)
        self.row2.pack(fill=tk.X, padx=5, pady=5)

        self.btn_github = tk.Button(
            self, text="打开我的GitHub主页", command=self.open_github)
        self.btn_github.pack(side=tk.BOTTOM, fill=tk.X, padx=5, pady=5)

    def select_file(self):
        txt_file = filedialog.askopenfile(
            initialdir=".", title="选择文件", filetypes=(('文件文件', '*.txt'), ('所有文件', '*.*')))
        if txt_file:
